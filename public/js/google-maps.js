(function(root, factory) {
  ...
  if (typeof define === "function" && define.amd) { define(factory) } else if (typeof exports === "object") { module.exports = factory() } else { root.GoogleMapsLoader = factory() } })(typeof window !== "undefined" ? window : null, function() { "use strict";
  ...
  GoogleMapsLoader.load = function(fn) {
    if (google === null) {
      if (loading === true) {
        if (fn) { callbacks.push(fn) } } else { loading = true;
  ...
  GoogleMapsLoader.createUrl = function() {
    ...
    if (GoogleMapsLoader.KEY) { url += "&key=" + GoogleMapsLoader.KEY }
    if (GoogleMapsLoader.LIBRARIES.length > 0) { url += "&libraries=" + GoogleMapsLoader.LIBRARIES.join(",") }
    if (GoogleMapsLoader.CLIENT) { url += "&client=" + GoogleMapsLoader.CLIENT + "&v=" + GoogleMapsLoader.VERSION }
  ...
  GoogleMapsLoader.release = function(fn) {
    var release = function() { GoogleMapsLoader.KEY = null;
      ...
      if (originalCreateLoaderMethod !== null) { GoogleMapsLoader.createLoader = originalCreateLoaderMethod;
        originalCreateLoaderMethod = null }
      if (script !== null) { script.parentElement.removeChild(script);
        script = null }
      if (fn) { fn() } };
    if (loading) { GoogleMapsLoader.load(function() { release() }) } else { release() } };
